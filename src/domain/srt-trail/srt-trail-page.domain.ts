...
export class SrtTrailPage extends CustomPage {
  ...
  private static readonly TRAIL_INQURY_CLICK_SELECTOR =
  ...
  public async setPage(
    ...
    await Promise.all([
      await this.setDepartStation(trainSearchParameters.departStationCode),
      await this.setArriveStation(trainSearchParameters.arriveStationCode),
  ...
  public async getTrailsInfo() {
    ...
    const tBodyHandle = await this.page.$$(
      SrtTrailPage.TRAILS_LIST_INQURY_SELECTOR,
    );
    const result = await Promise.all(
      tBodyHandle.map(async (tr) => {
